...
-- First check if duration column exists and add it if not
DO $$
BEGIN
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    UPDATE courses SET duration_days = 2 WHERE course_type = 'Paediatric' AND NOT (name LIKE '%Emergency%');
