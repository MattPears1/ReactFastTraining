...
interface AdminState {
  ...
  clearUser: () => void;
  updateSession: (token: string, refreshToken: string, expiresAt: string) => void;
  ...
  resetFilters: () => void;
  updateViewPreferences: (prefs: Partial<AdminState['viewPreferences']>) => void;
  ...
  devtools(
    persist(
      immer((set, get) => ({
        ...
        updateSession: (token, refreshToken, expiresAt) => set((state) => {
...
export const selectIsAuthenticated = (state: AdminState) => 
  !!state.currentUser && !!state.sessionToken && 
  (state.sessionExpiresAt ? new Date(state.sessionExpiresAt) > new Date() : false);
