...
import {
  ...
  Product,
  Order,
} from "@/services/analytics/ecommerce.service";
...
export const useEcommerceTracking = () => {
  ...
  const trackProductListView = useCallback(
    (products: Product[], listName: string) => {
  ...
  const trackAddShippingInfo = useCallback(
    (shippingTier: string, shippingCost: number) => {
  ...
  const trackRefund = useCallback(
    (orderId: string, products?: Product[], amount?: number) => {
  ...
  const trackProductSearch = useCallback(
    (searchTerm: string, resultsCount: number, products?: Product[]) => {
  ...
  const trackPromotion = useCallback(
    (
      ...
      ecommerceAnalytics.trackPromotion(
        promotionId,
        promotionName,
        creative,
        position,
  ...
  const trackPromotionClick = useCallback(
    (
      ...
      ecommerceAnalytics.selectPromotion(
        promotionId,
        promotionName,
        creative,
        position,
