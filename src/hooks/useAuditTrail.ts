...
export interface AuditAction {
  action: string;
  category: 'auth' | 'booking' | 'course' | 'user' | 'payment' | 'email' | 'settings' | 'security';
...
export const useAuditTrail = () => {
  ...
  const logAction = useCallback(async (auditAction: AuditAction) => {
    if (!user) return;
    ...
    try {
      ...
      if (auditAction.severity === 'critical' || auditAction.severity === 'error') {
...
if (typeof window !== 'undefined') {
  window.addEventListener('beforeunload', () => {
    if (auditLogQueue.length > 0) {
      ...
      const data = new Blob([JSON.stringify({ logs: auditLogQueue })], { type: 'application/json' });
