...
describe('useSearch', () => {
  ...
  it('deduplicates recent searches', async () => {
    mockLocalStorage.getItem.mockReturnValue(JSON.stringify(['services', 'products']))
    ...
    await waitFor(() => {
      ...
      expect(result.current.recentSearches.filter(s => s === 'services')).toHaveLength(1)
    })
  })

  it('clears recent searches', () => {
    mockLocalStorage.getItem.mockReturnValue(JSON.stringify(['search1', 'search2']))
    ...
    expect(result.current.recentSearches).toEqual([])
    expect(mockLocalStorage.setItem).toHaveBeenCalledWith('recent_searches', JSON.stringify([]))
  })
  ...
  it('searches by title, description, and category', async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.suggestions.every(s => s.category === 'faq')).toBe(true)
    })
  })
