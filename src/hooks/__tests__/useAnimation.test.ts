...
describe('useAnimation Hook', () => {
  ...
  describe('useFadeIn', () => {
    ...
    it('respects custom duration', () => {
      const { result } = renderHook(() => useAnimation.useFadeIn({ duration: 1000 }));
      
      expect(result.current.style.transition).toContain('1000ms');
    });

    it('applies delay when specified', () => {
      const { result } = renderHook(() => useAnimation.useFadeIn({ delay: 200 }));
  ...
  describe('useSlideIn', () => {
    ...
    it('slides in from specified direction', () => {
      ...
      directions.forEach(direction => {
        const { result } = renderHook(() => useAnimation.useSlideIn({ direction }));
    ...
    it('respects custom distance', () => {
      const { result } = renderHook(() => useAnimation.useSlideIn({ distance: 50 }));
  ...
  describe('useRotate', () => {
    it('rotates to specified angle', () => {
      const { result } = renderHook(() => useAnimation.useRotate({ angle: 180 }));
