...
class AdminAnalyticsService {
  ...
  async getComprehensiveAnalytics(
    ...
    try {
      const response = await fetch(
        `${this.baseURL}/api/admin/analytics/comprehensive?range=${timeRange}`,
        {
          headers: {
  ...
  async getCoursePopularity(days: number = 30): Promise<CoursePopularity[]> {
    try {
      const response = await fetch(
        `${this.baseURL}/api/admin/analytics/course-popularity?days=${days}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
          },
  ...
  async getRevenueByourse(days: number = 30): Promise<RevenueByourse[]> {
    try {
      const response = await fetch(
        `${this.baseURL}/api/admin/analytics/revenue-by-course?days=${days}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
          },
  ...
  async getDayOfWeekAnalysis(days: number = 30): Promise<DayOfWeekData[]> {
    try {
      const response = await fetch(
        `${this.baseURL}/api/admin/analytics/day-of-week?days=${days}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
          },
  ...
  async getBookingFunnel(days: number = 30): Promise<BookingFunnel> {
    try {
      const response = await fetch(
        `${this.baseURL}/api/admin/analytics/booking-funnel?days=${days}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
          },
