...
export const AnalyticsPage: React.FC = () => {
  ...
  const [timeRange, setTimeRange] = useState(searchParams.get('range') || '30days');
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const data = await adminAnalyticsService.getComprehensiveAnalytics(timeRange);
      ...
      <AdminCard title="Course Performance Details">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {analyticsData?.courseDetails?.map((course: any, index: number) => (
                <tr key={index} className="bg-white border-b hover:bg-gray-50">
                  <td className="px-6 py-4 font-medium text-gray-900">
                    {course.courseName}
                  </td>
                  <td className="px-6 py-4 text-center">{course.totalBookings}</td>
                  <td className="px-6 py-4 text-center">£{course.revenue.toLocaleString()}</td>
                  <td className="px-6 py-4 text-center">{course.avgAttendees}</td>
                  ...
                  <td className="px-6 py-4 text-center">{course.popularDay}</td>
                  <td className="px-6 py-4 text-center">
