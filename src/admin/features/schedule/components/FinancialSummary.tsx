...
export const FinancialSummary: React.FC<FinancialSummaryProps> = ({
  ...
  const financialMetrics = useMemo(() => {
    ...
    const totalPotentialRevenue = session.maxParticipants * session.pricePerPerson;
    const currentRevenue = paidBookings.reduce((sum, b) => sum + b.paymentAmount, 0);
    const pendingRevenue = pendingPayments.reduce((sum, b) => sum + b.paymentAmount, 0);
    const refundedAmount = refundedBookings.reduce((sum, b) => sum + b.paymentAmount, 0);
    ...
    >
      <div className="space-y-6">
        ...
        <div className="text-center">
          ...
          <p className={`text-4xl font-bold ${getRevenueColor(financialMetrics.revenuePercentage)}`}>
        ...
        <div className="border-t pt-4 space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between py-2">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium">{financialMetrics.paidCount}</span>
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium">{financialMetrics.pendingCount}</span>
                <span className="text-sm text-gray-500">
                  (£{financialMetrics.pendingRevenue.toLocaleString()})
                </span>
              </div>
            ...
            {financialMetrics.refundedCount > 0 && (
              <div className="flex items-center justify-between py-2">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium">{financialMetrics.refundedCount}</span>
                  <span className="text-sm text-gray-500">
        ...
        <div className="border-t pt-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium">{financialMetrics.revenuePercentage.toFixed(1)}%</p>
            </div>
            <div>
              <p className="text-gray-500">Avg. Booking Value</p>
              <p className="font-medium">£{financialMetrics.averageBookingValue.toFixed(2)}</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start space-x-2 p-3 bg-yellow-50 rounded-lg">
            ...
            <div className="text-sm">
              ...
              <p className="text-yellow-700">
                {financialMetrics.pendingCount} payments totaling £{financialMetrics.pendingRevenue.toLocaleString()} are awaiting confirmation.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-start space-x-2 p-3 bg-red-50 rounded-lg">
            ...
            <div className="text-sm">
              ...
              <p className="text-red-700">
                {financialMetrics.failedCount} payment(s) failed. Follow up with attendees.
              </p>
            </div>
          </div>
        )}
        ...
        {/* Action Buttons */}
        {(financialMetrics.refundedCount > 0 || session.status === 'CANCELLED') && onProcessRefunds && (
