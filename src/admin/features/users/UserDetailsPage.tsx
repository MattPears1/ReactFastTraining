...
export const UserDetailsPage: React.FC = () => {
  ...
  const getPaymentStatusBadge = (status: string): 'success' | 'warning' | 'error' | 'neutral' => {
    switch (status) {
  ...
  const getBookingStatusBadge = (status: string): 'success' | 'warning' | 'error' | 'neutral' => {
    switch (status) {
            ...
            {(user.address_line1 || user.city) && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {user.address_line1 && <p className="text-sm text-gray-900">{user.address_line1}</p>}
                  {user.address_line2 && <p className="text-sm text-gray-900">{user.address_line2}</p>}
                  <p className="text-sm text-gray-900">
                    {[user.city, user.county, user.postcode].filter(Boolean).join(', ')}
                  </p>
                </div>
              </div>
            )}
        ...
        >
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-900 capitalize">{user.customer_type || 'Individual'}</p>
            </div>
            
            <div>
              <p className="text-sm font-medium text-gray-600">Newsletter</p>
              <p className="text-sm text-gray-900">{user.newsletter_subscribed ? 'Subscribed' : 'Not Subscribed'}</p>
            </div>
        ...
        >
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-semibold text-gray-900">{user.total_bookings || 0}</span>
            </div>
            ...
            {user.total_bookings > 0 && (
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-gray-900">
                  £{(parseFloat(user.total_spent || 0) / (user.total_bookings || 1)).toFixed(2)}
                </span>
              </div>
            )}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {user.bookings.map((booking: any) => (
                  <tr key={booking.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">£{booking.payment_amount}</div>
                      <AdminBadge variant={getPaymentStatusBadge(booking.payment_status)}>
                        {booking.payment_status}
                      </AdminBadge>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <AdminBadge variant={getBookingStatusBadge(booking.status)}>
                        {booking.status}
                      </AdminBadge>
                    </td>
                  </tr>
