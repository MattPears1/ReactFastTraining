...
export const NextCourseCard: React.FC<NextCourseCardProps> = ({
  course,
  onUpdate,
}) => {
  ...
  const downloadPreMaterials = async () => {
    try {
      const materials = await clientPortalService.getPreCourseMaterials(
        course.booking.id,
      );
  ...
  return (
    <div className={`rounded-lg border-2 p-4 sm:p-6 mb-6 ${getUrgencyColor()}`}>
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-sm sm:text-base text-gray-700 dark:text-gray-300">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {course.session.startTime} - {course.session.endTime}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {course.attendeeCount} attendee
                {course.attendeeCount !== 1 ? "s" : ""}
              </span>
            </div>
          </div>

          {course.specialRequirements &&
            course.specialRequirements.length > 0 && (
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                      We have your requirements on file and will ensure
      ...
      {course.isToday && (
        <CourseCountdown startTime={course.session.startTime} />
      )}
    </div>
  );
};
