...
export const BookingDetailModal: React.FC<BookingDetailModalProps> = ({
  ...
  const addToCalendar = async () => {
    ...
    try {
      const icsUrl = await clientPortalService.addToCalendar(booking.booking.id);
                        ...
                        <ul className="space-y-2">
                          {booking.requirements.map((req, index) => (
                            <li key={index} className="text-sm">
                              <strong className="capitalize">{req.category}:</strong> {req.requirementType}
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {booking.attendees.map((attendee) => {
                      const attendance = booking.attendance.find(a => a.userId === attendee.id);
                      return (
                        <div
                          key={attendee.id}
