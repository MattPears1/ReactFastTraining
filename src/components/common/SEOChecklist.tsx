...
const SEOChecklist: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const runSEOChecks = () => {
      ...
      newChecks.push({
        id: 'title',
        label: 'Page Title',
        status: title && title.textContent && title.textContent.length > 0 ? 'pass' : 'fail',
      ...
      newChecks.push({
        ...
        status: descriptionContent && descriptionContent.length >= 50 ? 'pass' : 
                descriptionContent && descriptionContent.length > 0 ? 'warning' : 'fail',
      ...
      newChecks.push({
        ...
        status: ogTitle && ogDescription && ogImage ? 'pass' : 
                ogTitle || ogDescription || ogImage ? 'warning' : 'fail',
      ...
      newChecks.push({
        ...
        status: imagesWithoutAlt.length === 0 ? 'pass' : 
                imagesWithoutAlt.length < images.length / 2 ? 'warning' : 'fail',
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      >
        SEO Checklist ({checks.filter(c => c.status === 'pass').length}/{checks.length})
      </button>
          ...
          <div className="space-y-2">
            {checks.map((check) => (
              <div key={check.id} className="flex items-start gap-2">
                {check.status === 'pass' && <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />}
                {check.status === 'warning' && <AlertCircle className="w-5 h-5 text-yellow-500 flex-shrink-0 mt-0.5" />}
