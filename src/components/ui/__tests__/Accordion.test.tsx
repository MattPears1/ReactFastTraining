...
const mockItems = [
  ...
  },
  {
    id: '3',
    title: 'What is Vite?',
    content: 'Vite is a build tool that aims to provide a faster and leaner development experience.',
  },
];
...
describe('Accordion Component', () => {
  ...
  it('expands item when clicked', async () => {
    ...
    const firstItemButton = screen.getByRole('button', { name: mockItems[0].title });
  ...
  it('allows multiple items to be open when multiple is true', async () => {
    ...
    const firstItemButton = screen.getByRole('button', { name: mockItems[0].title });
    const secondItemButton = screen.getByRole('button', { name: mockItems[1].title });
