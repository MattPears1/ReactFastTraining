...
import { Calendar, Users, Plus, ClipboardCheck, TrendingUp, AlertCircle, ChevronRight } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'create' | 'attendance' | 'overview'>('overview');
  ...
  const upcomingSessions = sessions.filter(s => 
    new Date(s.startDate) >= new Date() && s.status !== 'CANCELLED'
  );
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            ...
            <p className="text-sm text-gray-600 mt-1">Manage courses and track attendance</p>
          </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900 mt-1">{stats.averageAttendance}%</p>
                  </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600 mt-1">
                            <span>{new Date(session.startDate).toLocaleDateString('en-GB')}</span>
        ...
        {activeTab === 'attendance' && (
          <div>
            {selectedSession ? (
              ...
              <div className="space-y-6">
                <h2 className="text-lg font-semibold text-gray-900">Select a Session</h2>
                
                {/* Past Sessions (need attendance) */}
                <div className="bg-white rounded-lg shadow-sm border">
                  <div className="p-6 border-b">
                    <h3 className="font-medium text-gray-900">Past Sessions Requiring Attendance</h3>
                  </div>
                  <div className="divide-y">
                    {pastSessions.filter(s => !s.attendanceMarked).length === 0 ? (
                      <div className="p-6 text-center text-gray-500">
                        All attendance marked
                      </div>
                    ) : (
                      pastSessions.filter(s => !s.attendanceMarked).map(session => (
                        <div
                          key={session.id}
                          className="p-6 hover:bg-gray-50 cursor-pointer transition-colors"
