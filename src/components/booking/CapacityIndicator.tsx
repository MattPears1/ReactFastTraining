...
export const CapacityIndicator: React.FC<CapacityIndicatorProps> = ({
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {/* Progress Bar */}
      <div className={cn('w-full bg-gray-200 rounded-full overflow-hidden', currentSize.bar)}>
        <div
      ...
      {/* Text Info */}
      {showNumbers && (
        <div className={cn('flex items-center justify-between', currentSize.text, colors.text)}>
          <span className="flex items-center gap-1">
            <Users className={currentSize.icon} />
            <span className="font-medium">{current}/{maxCapacity} booked</span>
          </span>
