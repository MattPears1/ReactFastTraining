...
export const BookingForm: React.FC<BookingFormProps> = ({
  ...
  const [numberOfParticipants, setNumberOfParticipants] = useState(
    inquiryData?.numberOfPeople || 1,
  );
  const [showParticipantDetails, setShowParticipantDetails] = useState(false);

  const [formData, setFormData] = useState({
    contactName: inquiryData
  ...
  const updateParticipantCount = (delta: number) => {
    const newCount = Math.max(
      1,
      Math.min(maxParticipants, numberOfParticipants + delta),
    );
    setNumberOfParticipants(newCount);

    // Adjust participant details array
    if (newCount > participantDetails.length) {
      const newParticipants = Array(newCount - participantDetails.length)
        .fill(null)
        .map(() => ({
          name: "",
          email: "",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm text-green-800">
              <p className="font-medium mb-1">
                Your inquiry details have been loaded
              </p>
              <p>
                We've pre-filled this form with the information from your
                inquiry. Your place is being held for 24 hours.
              </p>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input
                  ...
                  onChange={(e) =>
                    updateParticipantDetail(index, "name", e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateParticipantDetail(index, "email", e.target.value)
                  }
                  className="input input-sm"
                />
                <input
      ...
      <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
        By confirming this booking, you agree to our terms and conditions and
        cancellation policy.
      </p>
    </form>
  );
