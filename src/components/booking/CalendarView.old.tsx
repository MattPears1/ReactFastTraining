...
import { 
  ...
  Users, AlertCircle, ChevronDown, ChevronUp 
} from 'lucide-react';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
      ...
      <div className="p-6 border-b dark:border-gray-700">
        ...
        <div className="grid grid-cols-7 gap-1 text-center">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => (
            <div key={day} className="text-sm font-medium text-gray-500 dark:text-gray-400 py-2">
              {day}
            </div>
          ))}
        </div>
          ...
          {calendarDays.map((day, index) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => hasCourses && !isPast && toggleDayExpanded(dateKey)}
              >
                <div className="flex items-start justify-between mb-1">
                  <span className={cn(
                    'text-sm font-medium',
                    isToday(day.date) && 'text-primary-600 dark:text-primary-400',
                ...
                {hasCourses && !isExpanded && !isPast && (
                  <div className="flex gap-1 mt-2 flex-wrap">
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            schedule.availableSpots === 0 ? 'bg-gray-300' : config.color.dot
                          )}
                    ...
                    {day.schedules.length > 3 && (
                      <span className="text-xs text-gray-500">+{day.schedules.length - 3}</span>
                    )}
                  </div>
                )}
              </motion.div>
      ...
      <AnimatePresence>
        {Array.from(expandedDays).map(dateKey => {
          const day = calendarDays.find(d => d.date.toDateString() === dateKey);
            ...
            >
              <div className="p-6">
                <h3 className="text-lg font-semibold mb-4">
                  Courses on {day.date.toLocaleDateString('en-GB', { 
                    weekday: 'long', 
                    day: 'numeric', 
                    month: 'long' 
                  })}
      ...
      <div className="p-6 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
        <div className="flex flex-wrap items-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600 dark:text-gray-400">Available courses</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600 dark:text-gray-400">Click dates to view courses</span>
          </div>
        </div>
      </div>
    </div>
