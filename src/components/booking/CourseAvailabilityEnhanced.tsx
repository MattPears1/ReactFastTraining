...
import { 
  Calendar, MapPin, Users, Clock, ChevronRight, AlertCircle, 
  RefreshCw, Grid3X3, CalendarDays, Filter, Search
} from 'lucide-react';
...
export const CourseAvailabilityEnhanced: React.FC<CourseAvailabilityEnhancedProps> = ({
  ...
  // Filter states
  const [selectedCourseType, setSelectedCourseType] = useState<CourseTypeCode | 'all'>(
  ...
  useEffect(() => {
    applyFilters();
  }, [schedules, selectedCourseType, selectedVenue, selectedMonth, searchQuery, showFullCourses]);
  ...
  if (loading) {
    return <BookingSkeleton variant={viewMode === 'calendar' ? 'grid' : 'list'} />;
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setSelectedCourseType(e.target.value as CourseTypeCode | 'all')}
          ...
          <div>
            ...
            <select
              value={selectedVenue}
              onChange={(e) => setSelectedVenue(e.target.value as VenueCode | 'all')}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500"
            >
              <option value="all">All Locations</option>
              {Object.entries(VENUE_CONFIG).map(([code, venue]) => (
                <option key={code} value={code}>{venue.name}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              {getAvailableMonths().map(month => {
                ...
                const monthName = new Date(Number(year), Number(monthNum) - 1).toLocaleDateString('en-GB', { 
                  month: 'long', 
                  year: 'numeric' 
                });
                return (
