...
export type PaymentStatusType = 'processing' | 'succeeded' | 'failed' | 'requires_action' | 'canceled';
...
export const PaymentStatus: React.FC<PaymentStatusProps> = ({ 
  ...
  const statusConfig = {
    ...
    succeeded: {
      ...
      message: amount ? `Your payment of £${amount} has been processed successfully.` : 'Your payment has been processed successfully.',
      showSpinner: false,
    },
    failed: {
      icon: XCircle,
    ...
    requires_action: {
      ...
      message: 'Your bank requires additional authentication. Please complete the verification process.',
      showSpinner: true,
    },
    canceled: {
      icon: XCircle,
      ...
      message: 'Your payment has been canceled. You can try again when you\'re ready.',
      showSpinner: false,
    },
  };
  ...
  return (
    <div id="payment-status" className={`${config.bgColor} ${config.borderColor} border rounded-lg p-6 transition-all duration-300`}>
      <div className="flex items-start gap-4">
        <div className="flex-shrink-0">
          {config.showSpinner ? (
            <div className="relative">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-600"></div>
              <Icon className={`absolute inset-0 m-auto w-6 h-6 ${config.iconColor}`} />
            </div>
          ) : (
        ...
        <div className="flex-1">
          <h3 className={`font-semibold text-lg mb-1 ${config.iconColor.replace('text-', 'text-')}`}>
            {config.title}
          </h3>
