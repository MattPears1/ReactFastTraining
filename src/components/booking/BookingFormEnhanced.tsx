...
import { 
  User, Mail, Phone, Building, Users, FileText, 
  Plus, Minus, AlertCircle, CheckCircle, Info,
  ChevronDown, ChevronUp
} from 'lucide-react';
import { useToast } from '@contexts/ToastContext';
import { cn } from '@utils/cn';
import { BookingFormData, validateBookingForm, formatValidationErrors } from '@/utils/validation/booking.validation';
...
export const BookingFormEnhanced: React.FC<BookingFormEnhancedProps> = ({
  ...
  const totalPrice = (formData.numberOfParticipants || 1) * courseSchedule.pricePerPerson;
  ...
  useEffect(() => {
    ...
    if (count > 1 && currentDetails.length !== count - 1) {
      const newDetails = Array(count - 1).fill(null).map((_, index) => 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await bookingService.createBooking(formData as BookingFormData);
      
      if (response.success && response.data) {
        showToast('success', 'Booking confirmed successfully!');
        onSuccess(response.data.confirmationCode, response.data.booking);
      } else {
        showToast('error', response.error || 'Booking failed. Please try again.');
      }
    } catch (error) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-primary-50 dark:bg-primary-900/20 rounded-lg p-4 mb-6">
        ...
        <div className="text-sm text-primary-700 dark:text-primary-300 space-y-1">
          <p>Date: {new Date(courseSchedule.startDate).toLocaleDateString('en-GB', { 
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => handleInputChange('numberOfParticipants', Math.max(1, (formData.numberOfParticipants || 1) - 1))}
          ...
          <div className="text-center">
            <div className="text-3xl font-bold">{formData.numberOfParticipants}</div>
            <div className="text-sm text-gray-600">participant{(formData.numberOfParticipants || 1) > 1 ? 's' : ''}</div>
          </div>
          
          <button
            type="button"
            onClick={() => handleInputChange('numberOfParticipants', Math.min(courseSchedule.availableSpots, (formData.numberOfParticipants || 1) + 1))}
            disabled={formData.numberOfParticipants === courseSchedule.availableSpots}
            className="p-2 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <AnimatePresence>
        {showGroupBooking && formData.participantDetails && formData.participantDetails.length > 0 && (
          <motion.div
            ...
            <div className="space-y-4">
              {formData.participantDetails.map((participant, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <h4 className="font-medium mb-3">Participant {index + 2}</h4>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateParticipantDetail(index, 'firstName', e.target.value)}
                        className={cn(
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateParticipantDetail(index, 'lastName', e.target.value)}
                        className={cn(
                          "w-full px-3 py-2 border rounded-lg text-sm",
