...
export const DashboardPage: React.FC = () => {
  const { stats, nextCourse, upcomingCourses, loading, error, refreshData } =
    useClientPortal();
  ...
  React.useEffect(() => {
    if (!loading) {
      if (nextCourse) {
        announce(
          `Dashboard loaded. Your next course is ${nextCourse.session.courseType} on ${new Date(nextCourse.session.sessionDate).toLocaleDateString()}`,
          "polite",
        );
      } else {
  ...
  const handleRetry = React.useCallback(async () => {
    try {
      await ErrorRecovery.retry(refreshData, {
        maxAttempts: 3,
        ...
        <div className="space-y-8">
          ...
          {upcomingCourses.length > 1 && (
            <div className="mt-8">
              ...
              <CourseList courses={upcomingCourses} onUpdate={refreshData} />
            </div>
          )}
        </div>
      </LoadingState>
