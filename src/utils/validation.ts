...
export const sessionBookingSchema = z.object({
  ...
  participantCount: z
    ...
    .max(BUSINESS_RULES.MAX_PARTICIPANTS, `Maximum ${BUSINESS_RULES.MAX_PARTICIPANTS} participants allowed`),
...
export const createSessionSchema = z.object({
  ...
  startDate: createDateValidation({
    minDate: new Date(Date.now() + BUSINESS_RULES.MIN_BOOKING_ADVANCE_HOURS * 60 * 60 * 1000),
    maxDate: new Date(Date.now() + BUSINESS_RULES.MAX_BOOKING_ADVANCE_DAYS * 24 * 60 * 60 * 1000),
  }),
...
export const validateBookingCapacity = async (
  ...
  if (currentCapacity.current + requestedParticipants > BUSINESS_RULES.MAX_PARTICIPANTS) {
    return {
      valid: false,
...
export const validateSessionTiming = (
  ...
  const maxAdvanceTime = new Date(
    now.getTime() + BUSINESS_RULES.MAX_BOOKING_ADVANCE_DAYS * 24 * 60 * 60 * 1000
  );
