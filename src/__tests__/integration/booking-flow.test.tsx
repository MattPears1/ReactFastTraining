...
describe("Booking Flow Integration", () => {
  ...
  describe("Session Availability", () => {
    it("should display available sessions with correct capacity", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Emergency First Aid at Work"),
        ).toBeInTheDocument();
  ...
  describe("Course Filtering", () => {
    it("should filter sessions by course type", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByText("Paediatric First Aid"),
  ...
  describe("Admin Course Creation", () => {
    it("should create a new course session", async () => {
      ...
      vi.mocked(adminApi.createSession).mockResolvedValue({
        id: "new-session-id",
      });

      const { container } = render(<CourseCreationForm />, {
        wrapper: createWrapper(),
      });

      await waitFor(() => {
        expect(
          screen.getByText("Emergency First Aid at Work (Full Day (6 hours))"),
        ).toBeInTheDocument();
      });

      // Fill form
      fireEvent.change(screen.getByLabelText(/course type/i), {
        target: { value: "efaw" },
      });
      fireEvent.change(screen.getByLabelText(/trainer/i), {
        target: { value: "lex-trainer-id" },
      });
      ...
      fireEvent.change(screen.getByLabelText(/price per person/i), {
        target: { value: "75" },
      });
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/session.*created successfully/i),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Attendance Marking", () => {
    it("should mark attendance for a session", async () => {
      ...
      vi.mocked(adminApi.getSessionAttendance).mockResolvedValue(
        mockAttendance,
      );
      ...
      const johnPresentButton = screen.getAllByRole("button", {
        name: /present/i,
      })[0];
      fireEvent.click(johnPresentButton);

      // Mark Jane as absent
      const janeAbsentButton = screen.getAllByRole("button", {
        name: /absent/i,
      })[1];
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/attendance saved successfully/i),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Real-time Updates", () => {
    it("should update capacity in real-time when bookings change", async () => {
      ...
      const { rerender } = render(<FilteredCourseList />, {
        wrapper: createWrapper(),
      });
