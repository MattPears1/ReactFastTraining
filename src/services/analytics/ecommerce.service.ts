...
class EcommerceAnalytics {
  ...
  addToCart(product: Product, quantity: number = 1) {
    ...
    if (existingIndex >= 0) {
      this.cart.items[existingIndex].quantity = (this.cart.items[existingIndex].quantity || 0) + quantity
    } else {
  ...
  trackPromotion(promotionId: string, promotionName: string, creative?: string, position?: string) {
    analytics.trackEvent({
      category: 'promotion',
      action: 'view',
      label: promotionName,
  ...
  selectPromotion(promotionId: string, promotionName: string, creative?: string, position?: string) {
    analytics.trackEvent({
      category: 'promotion',
      action: 'select',
      label: promotionName,
  ...
  // Search tracking
  searchProducts(searchTerm: string, resultsCount: number, products?: Product[]) {
    ...
    if (products && products.length > 0) {
      analytics.trackEvent({
        ...
        properties: {
          ...
          items: products.slice(0, 10).map(product => this.productToItem(product))
        }
      })
    }
  }
