...
class ConversionTracking {
  ...
  private setupDefaultGoals() {
    ...
    this.addGoal({
      ...
      value: 50,
      conditions: [
        { type: 'event', operator: 'equals', value: 'user_signup' }
      ]
    })
    ...
    this.addGoal({
      ...
      conditions: [
        { type: 'event', operator: 'equals', value: 'purchase' }
      ]
    })
  ...
  trackFunnelStep(funnelId: string, stepIndex: number, abandoned: boolean = false) {
  ...
  trackABTestConversion(testId: string, variant: string, goalId: string, value?: number) {
    analytics.trackEvent({
      category: 'ab_test',
      action: 'conversion',
