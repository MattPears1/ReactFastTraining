...
class RequestOptimizer {
  ...
  async prefetch<T>(
    ...
    await this.get<T>(url, { ...config, cacheDuration: config.cacheDuration || this.defaultCacheDuration });
  }
  ...
  getCacheStats(): {
    ...
    return {
      ...
      newestEntry: timestamps[timestamps.length - 1] ? new Date(timestamps[timestamps.length - 1]) : undefined,
    };
  }
